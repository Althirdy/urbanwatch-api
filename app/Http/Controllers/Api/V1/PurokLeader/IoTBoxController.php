...
class IoTBoxController extends BaseApiController
{
    ...
    public function storeAnomalyLog(Request $request)
    {
        ...
        try {
            // Check if IoT box is registered
            $iotBox = UwDevice::where('device_id', $request->device_id)
                ->where('status', 'active')
                ->first();

            if (! $iotBox) {
                Log::warning('Unregistered or inactive IoT box attempted to send data', [
                    'device_id' => $request->device_id,
                    'ip' => $request->ip(),
                ]);

                return $this->sendError('IoT box is not registered or is inactive.', null, 403);
            }
    ...
    public function verifyDevice(Request $request)
    {
        ...
        $iotBox = UwDevice::where('device_id', $request->device_id)->first();
